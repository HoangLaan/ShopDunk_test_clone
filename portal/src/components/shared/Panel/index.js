...
const Panel = ({ onSubmit, loading, panes, actions, noActions, purchaseOrderId, hasSubmit, ...props }) => {
  ...
  useEffect(() => {
    if (pathname === '/stocks-in-request/add' && isAdd && purchaseOrderId && tab_active === panes[0]?.key?.toLowerCase()) {
      setQuery({ ...query, purchase_order_id: purchaseOrderId }, true);
    }
  }, [pathname, isAdd, purchaseOrderId, tab_active]);
  ...
  const jsx_footer = useMemo(() => {
    ...
    if (Boolean(parseActionValue.length)) {
      return parseActionValue.map((props, i) => <BWButton key={i} className='mr-2' {...props} />);
    } else {
      if (isView) {
        return (
        ...
        {parsePanel.map((e) => {
          return (
            <li
              ...
              {e?.permission ? (
                <CheckAccess permission={e?.permission} >
