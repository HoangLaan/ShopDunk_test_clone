...
const StockoutInforPreOrder = ({ id, title, disabled }) => {
  const { watch, clearErrors, setValue, unregister, register } = useFormContext();
  ...
  const handleChangeStore = useCallback(
    (store_id) => {
      ...
      // clear imei
      let products = Object.values(watch('products') || {});
      products = products
        .map((item) => {
          return { ...item, imei_codes: [] };
        })
        .reduce((acc, item) => {
          return { ...acc, [item.product_id]: item };
        }, {});
  ...
  // disable receiving_date validate
  useEffect(() => {
    unregister('receiving_date');
  ...
  return (
    <BWAccordion title={title} id={id} isRequired>
      <div className='bw_row'>
        ...
        <FormItem label='Cửa hàng' className='bw_col_4' disabled isRequired>
          <FormDebouneSelect
            field='store_id'
            id='store_id'
            options={storeOpts}
        ...
        <FormItem disabled label='Ngày nhận hàng dự kiến' className='bw_col_4'>
          <FormDatePicker
            style={{
              width: '100%',
              padding: '2px 0px',
...
export default StockoutInforPreOrder;
