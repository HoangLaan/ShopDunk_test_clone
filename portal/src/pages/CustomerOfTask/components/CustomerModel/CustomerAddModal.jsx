...
import { createCustomerLead } from 'services/customer.service';
...
const CustomerAddModal = ({ onClose, getOptsCustomer, setValueCustomer }) => {
  ...
  useEffect(() => {
    ...
    if (_customerTypeOptions?.length > 0) {
      setDataTypeList(_customerTypeOptions)
      _customerTypeOptions?.sort((a, b) => parseInt(a?.orderindex) - parseInt(b?.orderindex));
      let smallestObject = _customerTypeOptions[0];
      if (smallestObject) {
        setValue('customer_type_id', smallestObject?.value);
      }

    }
  }, [customerTypeOptions, setValue])
  ...
  const onSubmit = async (payload) => {
    try {
      ...
      payload.is_system = 0;
      setLoading(true);
      const memberDetail = await createCustomerLead({ ...payload, phone_number: payload?.phone_number?.trim(), full_name_customer: payload?.full_name });
    ...
    <div className='bw_modal bw_modal_open' id='bw_add_customer'>
      <div className='bw_modal_container bw_w900 '>
        <div className='bw_title_modal'>
          <h3>Thêm mới khách hàng tiềm năng</h3>
            ...
            <div className='bw_main_modal'>
              <div className='bw_row'>
                <div className='bw_col_4'>
                  ...
                  <FormItem label='Số điện thoại' style='gray' isRequired={true}>
                    <FormInput
                      type='text'
                      field='phone_number'
                ...
                <div className='bw_col_4'>
                  <div style={{ height: '79px' }}>
                    <FormItem isRequired label='Giới tính' style='gray'>
                      <div className='bw_flex bw_align_items_center bw_lb_sex_padding'>
                ...
                <div className='bw_col_4'>
                  <FormItem isRequired label='Hạng khách hàng' style='gray'>
                    <FormSelect
                      list={dataTypeList?.map((p) => {
                        return {
                          label: p?.name,
                          value: p?.id,
                        };
                  ...
                  <FormItem label='Ngày/Tháng/Năm sinh' style='gray'>
                    <FormDatePicker
                      style={{
                        width: '100%',
                        padding: '2px 0px',
