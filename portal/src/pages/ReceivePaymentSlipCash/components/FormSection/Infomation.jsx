...
import { getCashierByCompanyId, getReceiveTypeOpts } from 'services/receive-slip.service';
import { getPaymentTypeOpts, getBusinessOpts } from 'services/receive-slip.service';
...
const InformationDetail = ({ disabled, title, objectName = 'chi', objectType, isAddPage }) => {
  ...
  // set default payment form values
  useEffect(() => {
    if (!getValues('payment_form_id') && paymentFormOptions?.some((_) => _.payment_type === PAYMENTTYPE.CASH)) {
      methods.setValue('payment_form_id', paymentFormOptions?.find((_) => _.payment_type === PAYMENTTYPE.CASH)?.value);
    }
  }, [paymentFormOptions]);
              ...
              <FormItem className='bw_col_4' label='Công ty' disabled={disabled} isRequired>
                <FormSelect
                  ...
                  onChange={(value) => {
                    ...
                    // getCashierByCompanyId({ company_id: value }).then((data) => {
                    //   setOptionsCashier(mapDataOptions4Select(data));
                    // });
        ...
        <div className='bw_row'>
          <FormItem label={`Nội dung ${objectName}`} className='bw_col_6' disabled={disabled}>
            <FormTextArea
              field='descriptions'
              validation={{
                required: 'Nội dung thu là bắt buộc',
              }}
            />
          </FormItem>
                ...
                <div className='bw_row'>
                  ...
                  <div className='bw_col_5'>
                    <TotalCountWrapper>
                      ...
                      <p className='number'>{formatPrice(methods.watch('total_money') || 0, false, ',')}</p>
                    </TotalCountWrapper>
                  </div>
                </div>
              </div>
