...
import { mapDataOptions, mapDataOptions4SelectCustom, showToast } from 'utils/helpers';
...
function PurchaseOrdersInfo({ disabled, isShowPaymentDetail }) {
  ...
  useEffect(() => {
    ...
    getBusinessOptions().then((data) => {
      setBusinessOptions(mapDataOptions4SelectCustom(data));
    });
  }, []);
    ...
    if (watch('company_id')) {
      getBusinessOptions({
        ...
        if (initialValue && !watch('business_id') && !watch('purchase_order_id')) {
          methods.setValue('business_id', Number(initialValue?.id));
        }
      });
    }
  }, [watch('company_id')]);
        ...
        <div className='bw_row'>
          <div className='bw_col_4'>
            <FormItem label='Mã đơn mua hàng' isRequired disabled>
              <FormInput type='text' field='purchase_order_code' />
            </FormItem>
          </div>
          <div className='bw_col_4'>
            <FormItem label='Loại yêu cầu nhập hàng' disabled={disabled}>
              <FormSelect
                type='text'
          ...
          <div className='bw_col_4'>
            <FormItem label='Chi nhánh' isRequired disabled={disabled}>
              <FormSelect
                field='business_id'
                list={mapDataOptions4SelectCustom(businessOptions)}
                validation={{
            ...
            <div className='bw_col_4'>
              <FormItem label='Hình thức thanh toán' disabled={disabled && isShowPaymentDetail}>
                <FormSelect
                  ...
                  disabled={disabled && isShowPaymentDetail}
                />
              </FormItem>
            </div>
          )} */}
