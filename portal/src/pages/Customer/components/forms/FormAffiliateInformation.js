...
import { getOptionsCustomerType, getOptionsSource } from 'services/customer.service';

function FormAffiliateInformation({ disabled, title }) {
  const [optionsSource, setOptionsSource] = useState([]);
  const [optionsCustomerType, setOptionsCustomerType] = useState([]);

  useEffect(() => {
    const getDataOptions = async () => {
      const _source = await getOptionsSource();
      setOptionsSource(mapDataOptions4Select(_source));

      const _customerType = await getOptionsCustomerType();
      ...
      <div className='bw_row'>
        <div className='bw_col_4'>
          <FormItem label='Mã khách hàng' disabled={true}>
            <FormInput field='customer_code' disabled={disabled} />
          </FormItem>
        </div>
        <div className='bw_col_4'>
...
export default FormAffiliateInformation;
