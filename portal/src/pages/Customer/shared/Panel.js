...
const Panel = ({ onSubmit, loading, panes, actions, noActions, hasSubmit, ...props }) => {
  ...
  const jsx_submit = !noActions && (
    <div className='bw_btn_save bw_btn_group bw_flex bw_justify_content_right bw_align_items_center' style={{ zIndex:0 }}>
  ...
  const jsx_main = (
    <React.Fragment>
      ...
      {parsePanel.map((e) => {
        const { component: Component, key, renderWhenNoActive = true, ...props } = e;
        const isRender = tab_active.toLowerCase() === key?.toString()?.toLowerCase() || (renderWhenNoActive !== false)
        return isRender && (
