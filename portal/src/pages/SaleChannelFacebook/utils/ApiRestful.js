...
export default class ApiRestful {
  ...
  static convertApiErrData(data) {
    ...
    if (data) {
      if (Array.isArray(errArr) && Boolean(errArr.length)) {
        let errors = [];
        errArr.forEach((err) => {
  ...
  get(url, params, _config) {
    let config = Object.assign(
      ...
      { ..._config, cancelToken: this.cancelTokenSource.token } || { cancelToken: this.cancelTokenSource.token },
    );
  ...
  uploadFileGlobal(url, formData, callback) {
