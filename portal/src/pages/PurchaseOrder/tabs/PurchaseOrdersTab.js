...
import { PAYMENT_TYPE, STATUS_PAYMENT, STATUS_PURCHASE_ORDERS } from '../utils/constants';
...
const PurchaseOrdersAdd = ({ setpurchaseOrder, isReturned }) => {
  ...
  const defaultForm = {
    ...
    business_id: user.user_business ? String(user.user_business[0]) : null,
  };
  ...
  const onSubmit = async (payload) => {
    try {
      ...
      if (purchase_order_id) {
        ...
        window._$g.rdr(is_returned_goods ? '/purchase-orders-returned' : '/purchase-orders');
      }
    } catch (error) {
          ...
          if (response.request_purchase_id) {
            methods.reset({
              ...
              product_list: response.product_list
                ?.filter((product) => product.is_reviewed !== 0)
                .map((x) => ({ ...x, cost_price: x.rpo_price })),
  ...
  const actions = [
    ...
    // },
    {
      ...
      className: 'bw_btn bw_btn_warning',
      isShow: is_returned_goods
        ? !watch('is_enough_stocks_in_returned') && !isAdd
        : Boolean(purchase_order_id) && isShowWarehouseBtn,
    ...
    },
    {
      ...
      icon: 'fi fi-rr-add-document',
      className: 'bw_btn bw_btn_warning',
      isShow: is_returned_goods ? false : isShowStocksDetail && isDividedBtn,
    ...
    },
    {
      ...
      isShow: is_returned_goods
        ? disabled ||
          !(watch('order_status') === STATUS_PURCHASE_ORDERS.IMPORTED || watch('is_payments_status_id') !== 0)
        : !isShowPaymentDetail || !isShowStocksDetail,
