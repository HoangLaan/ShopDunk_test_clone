...
import { formatPrice, formatPricePurchaseOrder } from 'utils/index';
...
const PurchaseOrdersProductListTable = ({ disabled, loading }) => {
  ...
  useEffect(() => {
    const setExpectedDateDefault = () => {
      const defaultExpectedDate = product_list[0]?.expected_date;
      if (defaultExpectedDate) {
        product_list.forEach((item, index) => {
          if (!product_list[index]?.expected_date)
            methods.setValue(`product_list.${index}.expected_date`, defaultExpectedDate);
        });
      }
    };
  ...
  const calTotalMoney = (index) => {
    ...
    let totalMoney = (discountTotalPrice + (discountTotalPrice * vatValue) / 100);
    ...
    if (discountTotalPrice === 0) {
      ...
      totalMoney = (totalPrice + (totalPrice * vatValue) / 100);
    }
    ...
    () => [
      ...
      },
      {
        ...
        formatter: (_, index) => (
          <FormNumber
            ...
            //comment required nhập đơn giá (ko bắt buộc nhập đơn giá)
            // validation={{
      ...
      },
      {
        ...
        formatter: (_, index) => {
          ...
          const vatPrice = ((discountTotalPrice * vatValue) / 100);
          return formatPricePurchaseOrder(vatPrice);
        },
      },
      {
  ...
  const [total_quantity, totalPriceAll] = useDeepMemo(() => {
    ...
    let _totalPriceAll = product_list.reduce((total, item, index) => total + calTotalMoney(index), 0) / 100;
    _totalPriceAll = Math.round(_totalPriceAll) * 100; // làm tròn 15/01/2024
