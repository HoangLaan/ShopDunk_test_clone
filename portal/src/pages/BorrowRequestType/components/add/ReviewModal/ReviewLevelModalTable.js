...
import { deleteBorrowReviewLv } from 'services/borrow-request-rl.service';

import DataTable from 'components/shared/DataTable/index';
import BWAccordion from 'components/shared/BWAccordion';

function ReviewLevelModalTable({
  data,
  totalPages,
  itemsPerPage,
  page,
        ...
        onClick: (_, d) =>
          dispatch(
            showConfirmModal(
              ...
              async () => {
                await deleteBorrowReviewLv([_?.review_level_id]);
                onRefresh();
              },
            ),
          ),
    ...
    <BWAccordion title='Chọn mức duyệt'>
      <DataTable
        ...
        handleBulkAction={(dataSelect) => {
          ...
          methods.setValue(reviewLevelUserListField, list);
        }}
        fieldCheck='review_level_id'
        defaultDataSelect={methods.watch(reviewLevelUserListField)}
        hiddenDeleteClick
      />
    </BWAccordion>
  );
