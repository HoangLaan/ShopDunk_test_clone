...
export default function AttributesAdd({setIsOpenModal}) {
  const methods = useForm();
  const {setValue, setError, getValues, handleSubmit, watch, reset, register} = methods;
  const [optionsUnit, setOptionsUnit] = useState(null);
  const [alerts, setAlerts] = useState([]);

  useEffect(() => {
    reset({is_active: 1, is_material: 1});
  }, []);
    ...
    <div className='bw_modal bw_modal_open' id='bw_formAdd'>
      <div className='bw_modal_container bw_w800'>
        <div className='bw_title_modal'>
          ...
          <span className='fi fi-rr-cross-small bw_close_modal' onClick={() => setIsOpenModal(false)}></span>
        </div>
          ...
          <form onSubmit={onSubmit}>
            <div className='bw_main_modal'>
              ...
              <div className='bw_row'>
                ...
                <div className='bw_col_4'>
                  <label className='bw_checkbox'>
                    <FormInput type='checkbox' field='is_component' value={watch('is_component')}/>
                    <span></span>
                    Là thuộc tính linh kiện
                  </label>
                </div>
                <div className='bw_col_4'>
                  <label className='bw_checkbox'>
                    ...
                    <span></span>
                    Là thuộc tính nhóm nguyên liệu
                  </label>
                </div>
              </div>
            </div>
                  ...
                  watch('attribute_values').map((attr, i) => (
                    <tr key={i}>
                      ...
                      <td className='bw_sticky bw_action_table'>
                        <a href='#!' onClick={() => handleDeleteRow(i)} className='bw_btn_table bw_delete bw_red'>
            ...
            <div className='bw_footer_modal bw_mt_3'>
              <button className='bw_btn bw_btn_success' onClick={handleSubmit(onSubmit)}>
                <span className='fi fi-rr-check'></span> Thêm mới
              </button>
              <button className='bw_btn_outline bw_close_modal' onClick={() => setIsOpenModal(false)}>
                Đóng
              </button>
            </div>
          </form>
