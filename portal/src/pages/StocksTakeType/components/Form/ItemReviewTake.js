...
const ItemReviewTake = ({ keyUser, disabled, index, stocksTakeReviewOpts, resetIsCompleted, update, remove, item }) => {
  ...
  const getOptionsReview = () => {
    ...
    return (stocksTakeReviewOpts || []).map((k) => {
        ...
        <td className='bw_text_center'>
          {watch(`${keyUser}.is_auto_reviewed`) === 1 ? (
            ...
            <FormSelect
              ...
              mode='multiple'
              disabled={disabled || !watch(`${keyUser}.stocks_review_level_id`)}
