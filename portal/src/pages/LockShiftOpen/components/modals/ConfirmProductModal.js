...
export default function ConfirmProductModal({setIsOpenConfirmProduct, listProduct, onConfirm }) {
    ...
    <div className='bw_modal bw_modal_open'>
      <div className='bw_modal_container bw_w700'>
        <div className='bw_title_modal'>
            ...
            <span className='fi fi-rr-cross-small bw_close_modal' onClick={()=>setIsOpenConfirmProduct(false)}></span>
        </div>
        <div className='bw_main_modal'>
            ...
            {isDifferenceQuantity && <div className='bw_note'>
             ...
             <DataTable noSelect noPaging actions={[]} columns={columns} data={listProduct}/>}   
            </div>
          </div>
          <div className='bw_footer_modal'>
            <BWButton type='success' icon={ICON_COMMON.save} content={'Đồng ý'} onClick={()=>onConfirm()} />
            <BWButton type='danger' icon={ICON_COMMON.reject} content={'Cập nhật lại'} onClick={()=>setIsOpenConfirmProduct(false)} />
        </div>
      </div>
    </div>
  );
