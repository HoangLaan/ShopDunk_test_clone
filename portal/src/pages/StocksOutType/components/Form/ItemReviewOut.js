...
const ItemReviewOut = ({ index, disabled, item, update, remove, resetIsCompleted, stocksOutReviewOpts, keyUser }) => {
  ...
  const getOptionsReview = () => {
    let reviews = watch('stocks_out_review_level_list') || [];
    return (stocksOutReviewOpts || []).map((k) => {
  ...
  return (
    <React.Fragment>
      <tr>
        ...
        <td>
          {watch(`${keyUser}.is_auto_reviewed`) === (1 || true) ? (
