...
import { reviewBorrowRequest } from "services/borrow-request.service";
...
export default function ModalReview({ itemReviewLevel, setShowModalReview, onRefresh }) {
  ...
  const handleApprove = async (value) => {
    try {
      ...
      let idBorrowRequest = itemReviewLevel;
      if (!itemReviewLevel) {
        showToast.warning('Đã có lỗi xảy ra vui lòng kiểm tra lại!', ToastStyle);
        return;
      }
      await reviewBorrowRequest(idBorrowRequest, { is_review: value, review_note:  watch('note_review')});
