...
const StocksOutTableConfig = ({
  ...
  const handleSelectedAll = ({ target: { checked } }) => {
    ...
    const _removeItem = _dataSelect?.filter(
      (item) => !data.find((i) => i.product_imei_code === item?.product_imei_code),
    );
  ...
  const totalChecked = data.filter((_item) =>
    dataSelect.find((i) => i.product_imei_code === _item?.product_imei_code),
  ).length;
