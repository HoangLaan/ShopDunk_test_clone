...
import jsx_purchase_user from './jsx_render/PurchaseUser';
import jsx_address from './jsx_render/Address';
import jsx_receiver from './jsx_render/Receiver';
import jsx_supplier from './jsx_render/Supplier';
import jsx_export_user from './jsx_render/ExportUser';
...
const StocksOutRequestDetail = ({ disabled }) => {
  ...
  useEffect(() => {
    if (listStockOutType.length > 0) {
      setValue('stocks_out_type_id', listStockOutType.find((item) => item.is_return_supplier)?.value);
    }
  }, [listStockOutType]);
  ...
  useEffect(() => {
    // Set default info
    const { user_business, department_id } = user;
    setValue('business_id', user_business ? user_business[0] : null);
    setValue('department_id', department_id ? String(department_id) : null);
    setValue('request_user', user.user_name);
  }, []);
  ...
  const jsx_from_stocks = (
    <div className='bw_col_4'>
      <FormItem disabled={true} isRequired label='Kho xuất'>
        <FormSelect
          allowClear
