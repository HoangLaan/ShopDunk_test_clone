...
import ProductRow from 'pages/StocksOutRequest/components/add/product-handle/ProductRow';
import { Empty } from 'antd';
import { getProductByImei } from 'services/stocks-out-request.service';
import { getErrorMessage } from 'utils';

const StocksOutRequestProduct = ({ disabled, isTransferDiffBusiness, stocks_out_request_id }) => {
  ...
  React.useEffect(() => {
    methods.register('product_list');
  }, []);

  const { watch, setValue } = methods;
  const product_list = Object.values(methods.watch('product_list') ?? {}) ?? [];
  ...
  return (
    <BWAccordion title='Sản phẩm xuất kho'>
      {!disabled ? (
      ...
      <table
        ...
        <thead>
          <tr>
            ...
            {watch('order_id') && <th className='bw_text_center'>Kho xuất</th>}
