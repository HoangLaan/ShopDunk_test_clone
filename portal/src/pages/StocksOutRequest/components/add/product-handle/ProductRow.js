...
const ProductRow = ({ keyProduct, indexProduct, disabled, isAddPage, isTransferDiffBusiness, order_id }) => {
  ...
  useEffect(() => {
    let accId
    if (isAddPage && currentURL.includes('/add') && watch('is_transfer'))
    accId = debtAccountingAccountOpts?.find((_) => _.label === '1561')?.id;
    else {
      accId = debtAccountingAccountOpts?.find((_) => _.label === '632')?.id
    }

    methods.setValue(`${field}.debt_account_id`, methods.watch(`${field}.debt_account_id`) || accId );
  ...
  }, [creditAccountingAccountOpts, debtAccountingAccountOpts, isAddPage]);
          ...
          ?.map((_) => {
            if (methods.watch)
              return {
                ...
                debt_account_id: methods.watch(`${field}.debt_account_id`) || _?.debt_account_id,
              };
          });
      });
    },
  ...
  return (
    <tr>
      ...
      {order_id && <td>
        <Fragment>
          <span>{methods.watch(`${field}.stocks_name`)}</span>
        </Fragment>
      </td>}
      {!disabled && (
