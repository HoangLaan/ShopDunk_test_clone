...
const FilterManager = ({ onChange, shop_id }) => {
  ...
  const getListOptionsShopProfile = async () => {
    try {
      ...
      setListShopProfile(listLazada);
      methods.setValue(
        'shop_id',
        listLazada && listLazada.length > 0 && listLazada[0] && listLazada[0].shop_id ? listLazada[0].shop_id : null,
      );
      methods.setValue(
        'stock_id',
        listLazada && listLazada.length > 0 && listLazada[0] && listLazada[0].stock_id ? listLazada[0].stock_id : null,
      );
    } catch (error) {
      notification.error({ message: `Có lỗi xảy ra.` });
    }
            ...
            <div className='bw_frm_box bw_readonly'>
              <div className='bw_row'>
                ...
                <h5
                  ...
                  Liên kết hàng hoá từ sàn Lazada
                </h5>
              </div>
              <div className='bw_row'>
