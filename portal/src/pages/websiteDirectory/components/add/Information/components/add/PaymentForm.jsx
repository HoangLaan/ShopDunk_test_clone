...
import { handleChangeMoneyPaymentCommon, resetMoneyAndPromotion } from 'pages/websiteDirectory/helpers/utils';
import { formatPrice } from 'utils';
import { orderType } from 'pages/websiteDirectory/helpers/constans';
    ...
    <React.Fragment>
      ...
      {Boolean(watch('data_payment') && watch('data_payment').length) &&
        watch('data_payment').map(
          (_item, idx) =>
            !Boolean(disabled && !_item.is_checked && watch('payment_status') !== 2) && ( // trạng thái đơn hàng là thanh toán 1 phần -> edit được hình thức thanh toán
                    ...
                    <div className='bw_col_2'>
                      <label className={`bw_checkbox`}>
                        <FormInput
                          ...
                          disabled={disabled && watch('payment_status') !== 2} // nếu là đơn đang thanh toán -> cho phép chỉnh sửa
                          onChange={({ target }) => {
