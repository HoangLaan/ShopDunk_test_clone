...
import { orderType, paymentFormType } from './constans';
...
export function getPaymentMethodByPaymentList(paymentList, order_type) {
  if ([orderType.INSTALLMENT_OFFLINE, orderType.INSTALLMENT_ONLINE].includes(order_type)) {
    return 'CK';
  }
...
export const calculateProductDiscount = (orderDetail) => {
  ...
  for (let i = 0; i < product_list.length; i++) {
    ...
    product_list[i]._total_money_without_vat =
      (product_list[i].total_price / (100 + (product_list[i].value_vat || 0))) *
        100 *
        (1 - product_list[i]._discount_percent) || 0;

    // tính % vat sản phẩm
    product_list[i]._vat_percent = product_list[i].value_vat ? product_list[i].value_vat / 100 : 0;
...
const _getDataInvoiceFromOrder = async (order) => {
  ...
  const invoiceData = {
    ...
    OriginalInvoiceData: {
      RefID: order.order_no.trim(),
      ExchangeRate: 1,
      PaymentMethodName: getPaymentMethodByPaymentList(order.data_payment, order.order_type),
