...
const ShiftModelTable = ({
  ...
  const handleSelectedAll = ({ target: { checked } }) => {
    ...
    if (checked) {
      ...
      data.map((_item) => {
        _selected[_item.department_id] = _item;
      });
    }
  ...
  const handleSelected = (valueRender, checked) => {
    ...
    if (checked) {
      _selected[valueRender?.department_id] = valueRender;
    } else {
      delete _selected[valueRender?.department_id];
    }
        ...
        <td className='bw_sticky bw_check_sticky bw_text_center'>
          <label className='bw_checkbox'>
            <input
              ...
              type='checkbox'
              checked={itemSelected[valueRender?.department_id] ? true : false}
            />
            <span></span>
          </label>
        </td>
  ...
  const totalChecked = data.filter((_user) => itemSelected[_user?.department_id]).length;
