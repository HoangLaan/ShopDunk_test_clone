...
const ShiftModelTable = ({
  ...
  const handleSelectedAll = ({ target: { checked } }) => {
    ...
    if (checked) {
      ...
      data.map((_item) => {
        _selected[_item.shift_id] = _item;
      });
    }
  ...
  const handleSelected = (valueRender, checked) => {
    ...
    if (checked) {
      _selected[valueRender?.shift_id] = valueRender;
    } else {
      delete _selected[valueRender?.shift_id];
    }
      ...
      <tr>
        <td className='bw_sticky bw_check_sticky bw_text_center'>
          <label className='bw_checkbox'>
            <input
              ...
              type='checkbox'
              checked={itemSelected[valueRender?.shift_id] ? true : false}
            />
            <span></span>
          </label>
        </td>
        {columns?.map((column, key) => {
          if (column.formatter) {
            ...
            return (
              <td style={{textWrap: 'wrap'}} className={column?.classNameBody} key={`${keyRender}${key}`}>
  ...
  const totalChecked = data.filter((_user) => itemSelected[_user?.shift_id]).length;
