...
import { mapDataCategory } from 'pages/InstallmentForm/utils/helper';

const AddCategoryModal = ({ open, onClose, title }) => {
  const [params, setParams] = useState({ page: 1, itemsPerPage: 25 });
  ...
  useEffect(() => {
    ...
    getList(params)
      .then((data) => {
        setDataItem(mapDataCategory(data));
      })
      .finally(() => {
        setLoading(false);
      });
