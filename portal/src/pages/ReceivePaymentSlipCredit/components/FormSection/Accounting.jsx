...
import { RECEIPTSOBJECT } from '../../utils/constants';
...
const Accounting = ({ title, disabled, type, isAddPage }) => {
    ...
    if (
      ...
      if (type === RECEIVE_PAYMENT_TYPE.RECEIVESLIP) {
        append({
          ...
          debt_account: deptAccountingAccountOpts?.find((_) => _.name === '1121')?.id || null,
          credit_account: creditAccountingAccountOpts?.find((_) => _.name === '331')?.id || null,
          money: 0,
        });
      }
      if (type === RECEIVE_PAYMENT_TYPE.PAYMENTSLIP) {
        append({
          ...
          credit_account: creditAccountingAccountOpts?.find((_) => _.name === '1121')?.id || null,
          money: 0,
        });
      }
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      disabled: disabled,
      formatter: (_, index) => {
        const debtAccountValue = methods.watch(`accounting_list.${index}.debt_account`);
        if (debtAccountValue === undefined || debtAccountValue === null) {
          const defaultDebtAccountId = deptAccountingAccountOpts?.find((_) => _.name === '331')?.id;
          if (defaultDebtAccountId) {
              setValue(`accounting_list.${index}.debt_account`, defaultDebtAccountId)
          }
        }
        return(
          <FormSelect
