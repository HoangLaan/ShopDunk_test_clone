...
import { bookkeeping } from 'services/receive-slip.service';
...
const ReceiveSlipTable = ({
  ...
  const columns = useMemo(
    () => [
          ...
          if (p.is_book_keeping === 1 && p.review_status === 1) {
            ...
            return (
              <div style={money < 0 ? { color: 'red' } : {}} className='bw_text_right'>
                {money > 0 ? formatMoney(money, true) : `(${formatMoney(Math.abs(money), true)})`}
              </div>
            );
          } else {
            return '';
      ...
      },
      {
        ...
        formatter: (item) => <div>{item.bank_account_number}</div>,
        classNameHeader: 'bw_text_center',
      },
      {
    ...
    return [
      {
        ...
        type: 'danger',
        content: 'Ghi sổ',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_BOOKKEEPING',
        onClick: () => {
      ...
      },
      {
        ...
        type: 'warning',
        content: 'Nhập excel',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_IMPORT',
        onClick: () => importExcel(),
      },
      {
        globalAction: true,
        ...
        type: 'primary',
        content: 'Xuất excel',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_EXPORT',
        onClick: () => exportExcel(),
      },
    ];
  }, [selectedItems, exportExcel, importExcel]);
    ...
    return [
      {
        ...
        color: 'blue',
        title: 'Duyệt',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_REVIEW',
        hidden: (p) => {
          return (
      ...
      },
      {
        ...
        color: 'green',
        title: 'Copy',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_COPY',
        onClick: (p) => window._$g.rdr(`/receive-payment-slip-credit/copy/${p?.id}`),
      },
      {
        icon: 'fi fi-rr-print',
        color: 'black',
        title: 'In',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_PRINT',
        onClick: (p) => {
      ...
      },
      {
        ...
        color: 'blue',
        title: 'Chi tiết',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_VIEW',
        onClick: (p) => window._$g.rdr(`/receive-payment-slip-credit/detail/${p?.id}`),
      },
      {
        icon: 'fi fi-rr-edit',
        color: 'blue',
        ...
        hidden: (p) =>
          ...
          p.is_book_keeping === 1,
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_EDIT',
        onClick: (p) => window._$g.rdr(`/receive-payment-slip-credit/edit/${p?.id}`),
      },
      {
        icon: 'fi fi-rr-trash',
        color: 'red',
        title: 'Xóa',
        permission: 'SL_RECEIVE_PAYMENT_CREDIT_DEL',
        onClick: (_, d) =>
          dispatch(
            showConfirmModal(
  ...
  return (
    <div>
      <CustomeActions actions={globalActions} permission={'SL_RECEIVE_PAYMENT_CREDIT_ADD'} />
      <DataTable
        loading={loading}
        data={data}
        columns={columns}
