...
const sendToInside = async function (mail = {}, TYPE = 'OFFWORK') {
    const pool = await mssql.pool;
    const transaction = await new sql.Transaction(pool);
    try {
        await transaction.begin();
        const template = templateHelper.createMail(TYPE, mail);
        ...
        const resultMailCreate = await requestMailCreate
            ...
            .execute("SYS_MAILBOX_CreateMail_App");
        ...
        if (users && users.length) {
            for (let i = 0; i < users.length; i++) {
                ...
                const resultMailUser = await requestMailUser
                    ...
                    .execute("SYS_MAILBOX_CreateMailUser_App");
        ...
        const resultMailBoxCreate = await requestMailBoxCreate
            ...
            .input("ISDRAFT", 0)
            .input("CREATEDUSER", apiHelper.getValueFromObject(mail, "user_name"))
            .execute("SYS_MAILBOX_CreateMailBox_App");
