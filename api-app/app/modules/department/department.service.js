...
const getOptions = async (queryParams = {}) => {
    try {
        ...
        const pool = await mssql.pool;
        const data = await pool.request().input('KEYWORD', keyword).execute(`MD_DEPARTMENT_GetOptions`);

        return new ServiceResponse(true, '', departmentClass.options(data.recordset));
    } catch (e) {
        logger.error(e, {function: 'DepartmentService.getOptions'});
        return new ServiceResponse(false, e.message);
    }
};
