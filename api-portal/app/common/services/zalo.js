...
class Zalo {
    ...
    constructor() {
        ...
        this.apiZaloPay = axios.create({
            baseURL: process.env.URL_ZALOPAY,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
        });
        ...
        this.api.interceptors.request.use(
            async (config) => {
                ...
                const data = await pool
                    .request()
                    .query('SELECT TOP 1 ACCESSTOKEN FROM MD_TOKENSOCIALNETWORK WHERE ISZALO = 1');
    ...
    async sendZNSZaloPay(data) {
        return await this.apiZaloPay({
            method: 'POST',
            headers: {
                'x-client-id': data.xClientId,
                'x-request-id': data.xRequestId,
                'x-client-hash': data.xClientHash
            },
            data: data.dataBody
        }).then((res) => res.data)
