...
const { authorizationVoip } = require('./utils/helpers');
const axios = require('axios');
const config = require('../../../config/config');
const sql = require('mssql');
const voipClass = require('./voip.class');
const qs = require('qs');
...
const exportExcel = async (queryParams = {}, bodyParams = {}) => {
    try {
        ...
        addSheetGetList({
            workbook: wb,
            sheetName: 'Lịch sử cuộc gọi ShopDunk',
            header: {
...
const getVoipExt = async (queryParams = {}) => {
    try {
        ...
        const data = await pool
            ...
            .execute('MD_VOIPEXT_GetList_AdminWeb');

        const voipExt = data.recordset;
        return new ServiceResponse(true, '', {
            data: voipClass.listVoipExt(voipExt),
            page: currentPage,
            limit: itemsPerPage,
            total: apiHelper.getTotalData(voipExt),
        });
    } catch (error) {
        logger.error(error, { function: 'voipService.getVoipExt' });
        return new ServiceResponse(false, '', {});
    }
};
