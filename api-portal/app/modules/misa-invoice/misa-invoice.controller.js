...
const publishTransportHSM = async (req, res, next) => {
    try {
        const serviceRes = await service.publishTransportHSM(req.body, req.query.store_id);
        if (serviceRes.isFailed()) {
            return next(serviceRes);
        }
        return res.json(new SingleResponse(serviceRes.getData(), 'success'));
...
const viewInvoiceByTransactionId = async (req, res, next) => {
    try {
        const serviceRes = await service.viewInvoiceByTransactionId(req.params?.transaction_id, req.body.auth_name);
        if (serviceRes.isFailed()) {
            return next(serviceRes);
        }
        return res.json(new SingleResponse(serviceRes.getData(), 'success'));
...
const saveInvoice = async (req, res, next) => {
    try {
        const serviceRes = await service.saveInvoice(req.body);
        if (serviceRes.isFailed()) {
            return next(serviceRes);
        }
        return res.json(new SingleResponse(serviceRes.getData(), 'success'));
...
const viewDemoInvoice = async (req, res, next) => {
    try {
...
const viewStocksOut = async (req, res, next) => {
    try {
