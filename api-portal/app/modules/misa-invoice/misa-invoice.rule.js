...
const ORIGINAL_INVOICE_DATA_SCHEMA = {
    ...
    InvSeries: Joi.string().max(10).default('1C24THM'), //ký hiệu
    ...
    DiscountRate: Joi.number().allow([null]).default(0),
    ...
    TotalDiscountAmountOC: Joi.number().allow([null]).default(0), // Tổng tiền CKTM
    TotalDiscountAmount: Joi.number().allow([null]).default(0), // Tổng tiền CKTM quy đổi
...
const ruleViewStocksOut = {
    ...
    InvSeries: Joi.string().default('6C24NHM'),
    ...
    InternalCommandDate: Joi.string(), // lệnh điều động ngày
...
const rulePublishTransportHSM = Joi.object({
    RefID: Joi.string().required(),
    OriginalInvoiceData: Joi.object(ruleViewStocksOut).required(),
    IsSendEmail: Joi.boolean().default(false),
    ReceiverName: Joi.string(),
    ReceiverEmail: Joi.string().email(),
    IsInvoiceSummary: Joi.boolean().default(false),
    stocks_out_request: Joi.number(),
});
