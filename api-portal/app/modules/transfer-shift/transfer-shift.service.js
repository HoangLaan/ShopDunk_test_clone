...
const getList = async (params = {}) => {
    try {
        ...
        const res = await pool
            ...
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_GETLIST_ADMINWEB);
        let trasferShifts = transferShiftClass.list(res.recordsets[0]);
        const list_user_review = transferShiftClass.listReview(res.recordsets[1]);
...
const createOrUpdate = async (id = null, params = {}) => {
    ...
    try {
        ...
        const dataCreateOrUpdate = await reqCreateOrUpdate
            ...
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_CREATEORUPDATE_ADMINWEB);
        ...
        if (id) {
            ...
            const dataDeleteReview = await reqDeleteReview
                ...
                .execute(PROCEDURE_NAME.HR_TRANSFERSHIFTREVIEWLIST_DELETEMANY_ADMINWEB);
        ...
        if (list_review.length > 0) {
            ...
            for (const element of list_review) {
                const dataCreateOrUpdateReview = reqCreateOrUpdateReview
                    ...
                    .execute(PROCEDURE_NAME.HR_TRANSFERSHIFTREVIEWLIST_CREATEORUPDATE_ADMINWEB);
        ...
        const data = await res
            ...
            .input('NEWSHIFTID', apiHelper.getValueFromObject(params, 'new_shift_id'))
            .input('USERNAME', apiHelper.getValueFromObject(params, 'auth_name'))
            .execute(PROCEDURE_NAME.HR_USER_SCHEDULE_TRANSFERSHIFT_ADMINWEB);
...
const detail = async (id) => {
    try {
        ...
        const res = await pool
            .request()
            .input('TRANSFERSHIFTID', id)
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_GETBYID_ADMINWEB);
...
const remove = async (bodyParams) => {
    ...
    try {
        ...
        for (let item of list_id) {
            const data = await reqDelete
                .input('TRANSFERSHIFTID', item)
                .input('DETELETEUSER', apiHelper.getValueFromObject(bodyParams, 'auth_name'))
                .execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_DELETE_ADMINWEB);
            if (!apiHelper.getResult(data.recordset)) {
                await transaction.rollback();
                return new ServiceResponse(false);
            }
        ...
        await transaction.rollback();
        logger.error(e, { function: 'transferShiftService.deleteManufacturer' });
        return new ServiceResponse(false, e.message);
    }
};
...
const updateReview = async (params = {}) => {
    try {
        ...
        const res = await pool
            ...
            .input('CREATEDUSER', apiHelper.getValueFromObject(params, 'auth_name'))
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFTREVIEWLIST_UPDATEREVIEW_ADMINWEB);
...
const getReviewInformation = async (params) => {
    try {
        ...
        const res = await pool
            ...
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFTREVIEWLIST_GETREVIEWINFORMATION_ADMINWEB);
...
const getShift = async (params) => {
    try {
        ...
        const res = await pool
            ...
            .input('CURRENTSHIFTID', apiHelper.getValueFromObject(params, 'current_shift_id'))
            .input('CURRENTSTOREID', apiHelper.getValueFromObject(params, 'current_store_id'))
            .input('ISSUPPORT', apiHelper.getValueFromObject(params, 'is_support'))
            .execute(PROCEDURE_NAME.HR_USER_SCHEDULE_GETSHIFT_ADMINWEB);
...
const getListReview = async (params) => {
    try {
        ...
        const res = await pool
            .request()
            .input('TRANSFERSHIFTTYPEID', apiHelper.getValueFromObject(params, 'transfer_shift_type_id'))
            .execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_LEVEL_USER_GETLISTBYTRANSFERSHIFTTYPE_ADMINWEB);
...
const getListTransferShiftType = async () => {
    try {
        const pool = await mssql.pool;
        const res = await pool.request().execute(PROCEDURE_NAME.HR_TRANSFERSHIFT_TYPE_GETALL_AMDINWEB);
...
const getOptionBusiness = async (queryParams = {}) => {
    try {
        ...
        const data = await pool
            ...
            .execute('HR_TRANSFERSHIFT_GetBusinessOptions_AdminWeb');
        const data_business = transferShiftClass.option(data.recordsets[0]);

        return new ServiceResponse(true, '', data_business);
    } catch (e) {
        logger.error(e, { function: 'transferShiftService.getOptionBusiness' });
        return new ServiceResponse(false, '', {});
    }
};
module.exports = {
