...
const getListAll = async (req, res, next) => {
  try {
    ...
    const listResponse = new ListResponse(data, total, page, limit);
    listResponse.data.parentFolder = parentFolder;
    listResponse.data.pathArray = pathArray;
...
const createDocumentType = async (req, res, next) => {
  try {
    const serviceRes = await FileManagerService.createDocumentType(req.body);
    if (serviceRes.isFailed()) {
      return next(serviceRes);
    }
    return res.json(new SingleResponse(serviceRes.getData(), serviceRes.getMessage()));
...
const UpdateDocumentType = async (req, res, next) => {
  try {
    const serviceRes = await FileManagerService.UpdateDocumentType(req.body);
    if (serviceRes.isFailed()) {
      return next(serviceRes);
    }
    return res.json(new SingleResponse(serviceRes.getData(), serviceRes.getMessage()));
...
const deleteDocumentType = async (req, res, next) => {
  try {
    ...
    const serviceRes = await FileManagerService.deleteDocumentType(document_type_id, req.body);
    if (serviceRes.isFailed()) {
      return next(serviceRes);
    }
    return res.json(new SingleResponse(serviceRes.getData(), serviceRes.getMessage()));
...
const downloadFile = async (req, res, next) => {
  try {
    ...
    let pathURL = `${file_url}`;
    return res.json(new SingleResponse(pathURL));
