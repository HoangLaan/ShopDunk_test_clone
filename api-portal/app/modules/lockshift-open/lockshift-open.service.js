...
const checkIsAllowOpenShift = async (userName) => {
  try {
    ...
    return new ServiceResponse(true, '', {isAllow});
  } catch (e) {
...
const checkHasPermission = async (userName) => {
  try {
    const pool = await mssql.pool;
    const responseData = await pool.request()
      .input('USERNAME', userName)
      .execute(PROCEDURE_NAME.MD_LOCKSHIFT_CHECK_HASPERMISSION);
    const isTrue = Boolean(responseData.recordset[0]?.RESULT) || false
    if(isTrue){
      return new ServiceResponse(true, 'Được quyền nhận ca', {isTrue});
    }
    else{
      return new ServiceResponse(true, 'Không có quyền nhận ca', {isTrue});
    }
  } catch (e) {
    logger.error(e, {function: 'LockShiftOpenService.checkHasPermission'});
    return new ServiceResponse(false, e.message);
  }
};
...
const createOrUpdateLockShiftOpen = async (body = {}) => {
  let lockShiftId = apiHelper.getValueFromObject(body, 'lock_shift_id')
  const listCashItems = apiHelper.getValueFromObject(body, 'list_shift_cash', [])
  const listProductItems = apiHelper.getValueFromObject(body, 'list_shift_product', [])
  const listEquipmentItems = apiHelper.getValueFromObject(body, 'list_shift_equipment', [])
  const listCustomerItems = apiHelper.getValueFromObject(body, 'list_shift_customer', [])
  ...
  try {
    ...
    if (!lockShiftId) {
      ...
      const shiftOpen = await requestShift
        ...
        .input('USERNAME', splitData(apiHelper.getValueFromObject(body, 'created_user')))
        .input('SHIFTLEADER', splitData(apiHelper.getValueFromObject(body, 'shift_leader')))
    ...
    for (let i = 0; i < listProductItems.length; i++) {
      ...
      await request2
        ...
        .input('STOCKSID', listProductItems[i]?.stocks_id)
    ...
    await requestUpdateShift
      .input('LOCKSHIFTID', lockShiftId)
      .input('TOTALCASH', totalMoney)
      .input('NOTE', shiftNote)
      .input('STOREID',apiHelper.getValueFromObject(body, 'store_id',5))
...
const deleteProductInShift = async (authname,lockShiftProductListId) => {
 
  try { 
    const pool = await mssql.pool;
    const data = await pool
      .request()
      .input('LOCKSHIFTPRODUCTLISTID', lockShiftProductListId)
      .input('DELETEDUSER', authname)
      .execute(PROCEDURE_NAME.MD_LOCKSHIFT_PRODUCTLIST_DELETE_PRODUCT_IN_LOCKSHIFT_ADMINWEB);
    return new ServiceResponse(true,"delete success",{})
  } catch (e) {
    logger.error(e, {function: 'lockShiftOpenService.deleteProductInShift'});
    return new ServiceResponse(false, e.message);
  }
}
