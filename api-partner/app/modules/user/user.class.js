...
const detail = (user) => {
  return transform.transform(user, [
    ...
    'user_name', 'first_name', 'last_name', 'birthday',
    'province_id', 'district_id', 'country_id', 'department_id',
    'position_id', 'ward_id', "entry_date",
    'province_name', 'district_name', 'country_name', 'ward_name', 'department_name', 'position_name', 'business_name',
    'user_companies', 'is_notify'
  ]);
};

const statistic = (user) => {
  return transform.transform(user, ['total_revenue', 'total_customer_create_order', 'total_netweight', 'total_debit', 'total_order']);
};


const list = (users = []) => {
  return transform.transform(users, [
    'user_id', 'user_name', 'full_name', 'department_name', 'position_name', 'default_picture_url'
  ]);
};
...
const getOptions = (data = []) => {
  return transform.transform(data, [
    'user_name', 'full_name', 'email', 'default_picture_url'
  ]);
};
...
const userGroups = (groups = []) => {
  ...
  return transform.transform(groups, [
      'id', 'user_id', 'value']);
};
